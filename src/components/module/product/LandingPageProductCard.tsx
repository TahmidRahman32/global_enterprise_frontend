         ...
         <div className="p-5">
            ...
            <div className="mt-3 flex items-center justify-between">
               ...
               <Link href={`/product/${product?.id}`}>
                  {" "}
                  <button className="rounded-full bg-yellow-700 px-4 py-1.5 text-sm font-medium text-white transition-colors hover:bg-yellow-600">Shop Now</button>
               </Link>
            </div>
         </div>
      </motion.div>
      ...
      <section className="w-full py-16 md:py-24 bg-background">
         <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.5 }} className="flex justify-center mt-12 md:mt-16">
               <Link href={"/product"}>
                  <button className="group relative inline-flex items-center justify-center overflow-hidden rounded-full bg-yellow-600 px-8 py-3 text-base font-semibold text-white transition-all duration-300 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2">
